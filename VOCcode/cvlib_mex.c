/*
 *      Revision 16.0 - ... Ismail Ari
 *      Contact info: http://ismailari.com
 *
 *      Revision 1.0 - 15.0 Joaquim Luis
 *
 ...
 *
 * Revision 16.0  10/06/2007 IA Added facedetect function
...
void Jfacedetect(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]);
...
void mexFunction(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
	...
	else if (!strcmp(funName,"facedetect"))
		Jfacedetect(n_out, plhs, n_in, prhs);
...
void Jfacedetect(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
    /*  */
	int nx, ny, nBands; /* width, height and # of channels in the given image */
    int nBands_out, nBytes, img_depth, out_dims[3];
    double *faceData;

	IplImage *src_img = 0; /* source image */
    IplImage *gray; /* We will use the gray version of the source image */
	mxArray *ptr_in, *ptr_out;
    
    /* Haar-classifier related variables */
    CvMemStorage* storage = 0;
    CvHaarClassifierCascade* cascade = 0;
    char* cascade_name = "haarcascades/haarcascade_frontalface_default.xml"; /* Let this be the default */
    int min_face_size = 20; /* The default, it is 20 */
    int print_detection_time = 0;
    ...
    /* Take the haar classifier file input if it is given */
    if(n_in > 2) {
        if(!mxIsChar (prhs[2]))
            mexErrMsgTxt ("Expecting Haar-classifier cascade filename as string in the second argument.");
        else {            
            cascade_name = mxArrayToString (prhs[2]);
        }
    }
    
    /* Take the min_face_size input if it is given */
    if(n_in > 3) {
        if(!mxIsDouble (prhs[3]))
            mexErrMsgTxt ("Expecting a number for min_face_size in the third input.");
        else {            
            min_face_size = *mxGetPr(prhs[3]); /* Get the given min face size */
        }
    }

    /* Take the print_detection_time option if it is given */
    if(n_in > 4) {
        if(!mxIsDouble (prhs[3]))
            mexErrMsgTxt ("Expecting a number for print_detection_time in the fourth input.");
        else {            
            print_detection_time = *mxGetPr(prhs[4]); /* Get the given min face size */
        }
    }

    if ( !(mxIsUint8(prhs[1]) || mxIsUint16(prhs[1]) || mxIsSingle(prhs[1])) )
		mexErrMsgTxt("Face detection error: Invalid image type. Valid types are: uint8, uint16 or single.\n");
	/* -------------------- End of parsing input ------------------------------------- */
	...
	Set_pt_Ctrl_out1 ( Ctrl, ptr_out ); 
	gray = cvCreateImageHeader( cvSize(nx, ny), img_depth, nBands_out );
	localSetData( Ctrl, gray, 2, nx * nBands_out * nBytes );

    cascade = (CvHaarClassifierCascade*)cvLoad( cascade_name, 0, 0, 0 );
    if(nBands == 1)
        cvCopyImage(src_img, gray); /* Copy the grayscale image */
    else
        cvCvtColor(src_img, gray, CV_RGB2GRAY); /* Convert the RGB image to grayscale */
    if( !cascade ) {
        mexErrMsgTxt( "Face detection error: Could not load classifier cascade file!\n" );
    }
    
    small_img = cvCreateImage( cvSize( cvRound (src_img->width/scale),
                         cvRound (src_img->height/scale)), 8, 1 );
    ...
    if( cascade ) {
        ...
        CvSeq* faces = cvHaarDetectObjects( small_img, cascade, storage,
                                            1.2, 3, CV_HAAR_DO_CANNY_PRUNING,
                                            cvSize(min_face_size, min_face_size) );
        t = (double)cvGetTickCount() - t;
        if( print_detection_time == 1 )
            mexPrintf( "Face detection time = %gms\n", t/((double)cvGetTickFrequency()*1000.) );
        plhs[0] = mxCreateDoubleMatrix( faces->total * 4, 1, mxREAL);
        faceData = mxGetPr(plhs[0]);
        ...
        for( i = 0; i < (faces ? faces->total : 0); i++ ) {
            ...
            faceData[i*4] = r->x * scale;
            faceData[i*4 + 1] = r->y * scale;
            faceData[i*4 + 2] = r->width * scale;
            faceData[i*4 + 3] = r->height * scale;
        }
    }

    cvReleaseImage( &small_img );
	cvReleaseImageHeader( &src_img );
    cvReleaseHaarClassifierCascade( &cascade ); /* Added by Albert Ali Salah to prevent memory leak */
	mxDestroyArray(ptr_in);
...
void facedetectUsage() {
	mexPrintf("Usage: ");
	mexPrintf("       Please refer to test_face_detect.m and face_detect.m files.\n");
}